...
import {
  ...
  BarChart3, Settings, LogOut, Shirt, ChevronLeft, Menu
} from 'lucide-react'
...
const navItems = [
  ...
  { to: '/kanban', icon: KanbanSquare, label: 'Produção' },
...
export function Sidebar({ collapsed, onToggle, mobileOpen, onMobileClose }) {
  ...
  const sidebarContent = (
    <div className="flex h-full flex-col">
      <div className={`flex items-center gap-3 px-5 py-6 border-b border-sidebar-hover ${collapsed ? 'justify-center' : ''}`}>
      ...
      <div className={`px-3 py-4 border-t border-sidebar-hover ${collapsed ? 'flex justify-center' : ''}`}>
        <button
          onClick={handleLogout}
          className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-sidebar-text hover:bg-sidebar-hover hover:text-white transition-all duration-200 cursor-pointer ${collapsed ? 'justify-center w-10 h-10 p-0' : 'w-full'}`}
        >
    ...
    <>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="fixed left-0 top-0 bottom-0 z-50 w-60 bg-sidebar shadow-2xl">
            {sidebarContent}
          </aside>
        </div>
      )}
      ...
      <button
        ...
        className="fixed top-4 left-4 z-30 flex h-10 w-10 items-center justify-center rounded-xl bg-card-bg border border-border shadow-sm lg:hidden cursor-pointer"
      >
